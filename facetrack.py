...
def main(session, faceSize):
    ...
    try:
        while True:
            ...
            if len(position) == 3:
                tracker_service.lookAt(position, 0, 0.5, True)
                tracker_service.pointAt("RArm", position, 0.5)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ip", type=str, default="138.67.236.222",
    ...
    session = qi.Session()
    # Replace args.ip with robot IP
    try:
        session.connect("tcp://" + "138.67.236.222" + ":" + str(args.port))
    except RuntimeError:
